...
import { Car, ChevronLeft, ChevronRight, Pencil, Plus, ReceiptText, Trash2, Utensils, WalletCards } from 'lucide-react-native';
      ...
      {message ? <View style={styles.center}><Text style={styles.error}>{message}</Text><Pressable onPress={() => void loadExpenses()}><Text style={styles.retry}>Tentar novamente</Text></Pressable></View> : loading ? <View style={styles.center}><ActivityIndicator color={colors.primary} /><Text style={styles.muted}>Carregando despesas...</Text></View> : (
        <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
          {expenses.length ? expenses.map((expense) => <View key={expense.id} style={styles.card}>
            ...
            <View style={styles.cardEnd}>{expense.category !== 'mileage' && <Text style={styles.amount}>{money(expense.amount)}</Text>}<View style={styles.cardActions}><Pressable accessibilityLabel="Editar despesa" onPress={() => router.push({ pathname: '/expenses/edit-expense', params: { id: String(expense.id) } } as any)} style={styles.actionButton}><Pencil size={17} color={colors.primary} /></Pressable><Pressable accessibilityLabel="Excluir despesa" onPress={() => remove(expense)} style={styles.actionButton}><Trash2 size={17} color={colors.danger} /></Pressable></View></View>
