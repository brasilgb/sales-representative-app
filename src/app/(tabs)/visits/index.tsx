...
import { CalendarDays, ChevronLeft, ChevronRight, LogIn, LogOut } from 'lucide-react-native';
  ...
  return (
    <View style={styles.screen}>
      <View style={styles.toolbar}>
        <View><Text style={styles.title}>Agenda de visitas</Text><Text style={styles.subtitle}>{visits.length} {visits.length === 1 ? 'compromisso' : 'compromissos'} no dia</Text></View>
      </View>
      ...
      {message ? <View style={styles.center}><Text style={styles.error}>{message}</Text><Pressable onPress={() => void loadVisits()}><Text style={styles.retry}>Tentar novamente</Text></Pressable></View> : loading ? <View style={styles.center}><ActivityIndicator color={colors.primary} /><Text style={styles.muted}>Carregando agenda...</Text></View> : (
        <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
          {visits.length ? visits.map((visit) => <VisitCard key={visit.id} visit={visit} onTransition={transition} />) : <View style={styles.center}><CalendarDays size={30} color={colors.mutedText} /><Text style={styles.emptyTitle}>Dia livre</Text><Text style={styles.muted}>As visitas programadas no sistema aparecerão aqui.</Text></View>}
        </ScrollView>
      )}
    </View>
  );
...
function VisitCard({ visit, onTransition }: { visit: VisitProps; onTransition: (visit: VisitProps, action: 'check-in' | 'check-out') => void }) {
  ...
  return (
    <View style={styles.card}>
      ...
      {visit.status === 'scheduled' ? <Pressable accessibilityLabel="Fazer check-in" onPress={() => onTransition(visit, 'check-in')} style={styles.action}><LogIn size={20} color={colors.primary} /></Pressable> : null}
      {visit.status === 'checked_in' ? <Pressable accessibilityLabel="Fazer check-out" onPress={() => onTransition(visit, 'check-out')} style={styles.action}><LogOut size={20} color={colors.success} /></Pressable> : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  toolbar: { minHeight: 78, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  action: { width: 44, height: 44, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surfaceRaised },
