      ...
      {items.length ? items.map((item) => (
        <View key={item.product_id} className="min-h-[68px] flex-row items-center gap-[9px] border-b border-white/10 px-3.5">
          <View className="min-w-0 flex-1">
            <Text className="text-[13px] font-extrabold text-foreground" numberOfLines={2}>{item.name}</Text>
            <Text className="mt-0.5 text-[11px] text-muted">{item.quantity} x {formatCurrency(item.price)}</Text>
            {item.discount_amount < 0 ? <Text className="mt-0.5 text-[11px] font-bold text-success">Desconto: − {formatCurrency(Math.abs(item.discount_amount))}</Text> : null}
            {item.discount_amount > 0 ? <Text className="mt-0.5 text-[11px] font-bold text-[#ffbd66]">Acréscimo: + {formatCurrency(item.discount_amount)}</Text> : null}
          </View>
