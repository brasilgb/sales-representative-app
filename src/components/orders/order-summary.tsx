    ...
    <View className="mb-4 overflow-hidden rounded-xl border border-white/10 bg-surface">
      ...
      {items.length ? items.map((item) => (
        <View key={item.product_id} className="min-h-[68px] flex-row items-center gap-[9px] border-b border-white/10 px-3.5">
          <View className="min-w-0 flex-1"><Text className="text-[13px] font-extrabold text-foreground" numberOfLines={2}>{item.name}</Text><Text className="mt-0.5 text-[11px] text-muted">{item.quantity} x {formatCurrency(item.price)}</Text></View>
          <Text className="text-xs font-black text-foreground">{formatCurrency(item.quantity * item.price)}</Text>
